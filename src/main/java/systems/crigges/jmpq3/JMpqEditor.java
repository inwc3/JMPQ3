 ...
 *
 * @author peq and Crigges Some basic basic pure java based mpq implementation
 *         to open and modify warcraft 3 archives. Any bugs report here:
...
public class JMpqEditor implements AutoCloseable {
	...
	public JMpqEditor(File mpqW) throws JMpqException {
		...
		try {
			...
			fc = FileChannel.open(tempMpq.toPath(), StandardOpenOption.CREATE, StandardOpenOption.READ,
	...
	public void close() throws IOException {
		...
		int currentPos = headerOffset + headerSize;// + newHashSize * 16 +
													// newBlockSize * 16;
		...
		channel.write(tempReader);
		channel.close();
		out.close();
		// FileChannel mpqChannel = FileChannel.open(mpqFile.toPath(),
		// StandardOpenOption.CREATE, StandardOpenOption.READ,
		// StandardOpenOption.WRITE);
		// mpqChannel.truncate(currentPos + 1);
		// MappedByteBuffer tempWriter = mpqChannel.map(MapMode.READ_WRITE, 0,
		// currentPos + 1);
