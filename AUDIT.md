...
- **P1-5a Correlate every locale mapping when enumerating blocks** *(found in review of the Phase 1 read side)*. `MpqArchive.entries()` names blocks by looking each list file entry up once, which resolves a single locale. An archive holding several localised variants of one path therefore reports the other variants with an empty name and locale 0, misstating their locale and leaving encrypted variants unreadable through the returned entry, since the key derives from the name. Fixing it needs the hash table to expose all mappings rather than one lookup per name, and no fixture has a multi-locale archive to test against, so it lands with P1-5 where a fixture can be built alongside it.
