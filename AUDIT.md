...
- **P2-4 `(attributes)` write support** *(issue [#11](https://github.com/inwc3/JMPQ3/issues/11))*. Honour the attributes bytemask properly on read (today hardcodes crc+timestamp layout and has a suspicious `-1` in the entry count — AttributesFile.java:38); regenerate CRC32+FILETIME on write when requested. Remove the dead commented block in `JMpqEditor.close`. Historical context from the issue thread: generation was disabled because CRC32 differed from StormLib for some `.wav` files — root cause is likely multi-compression handling (first sector of ADPCM-compressed wavs is not ADPCM-compressed since it holds the wav header). Fix alongside P2-6 and pin with a StormLib-golden CRC test. Acceptance for the issue itself: load + close `war3.mpq`-style archives without dropping `(attributes)` in a way the game rejects.
  ...
  - **The `.wav` CRC32 concern from the issue thread is reasoned about, not verified.** The argument is that the checksum is taken over decoded content, so it can only differ from StormLib if the decode differs, and the multi-compression ordering that would cause that was made table-driven in P2-6. That argument is probably right and is not evidence. The audit asked for this to be "pinned with a StormLib-golden CRC test" and that test does not exist, because **no fixture in the repository contains an ADPCM-compressed file at all** — the codec breakdown across all 186 fixture entries is 130 zlib, 38 stored, 14 pkware, 2 stored+zlib, 1 huffman, 1 empty. The write side cannot produce ADPCM either, so the fixture has to come from StormLib or a real map with sound. Until then, treat attributes generation as untested against the exact case that caused it to be disabled in 2017.
...
- **P4-3b ADPCM/Huffman fixture — outstanding** *(found while auditing issue [#11](https://github.com/inwc3/JMPQ3/issues/11))*. No fixture exercises ADPCM, so the wav decode path that issue #11 turns on is unverified, and `tools/mpqref.py` cannot check it either (it implements neither ADPCM nor Huffman). Needs a StormLib-generated archive containing a `.wav`, after which the reference should grow both codecs so the check is independent rather than a cross-comparison between this library's own two decoders.
